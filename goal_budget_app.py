...
if st.session_state.current_user:
    ...
    if not user_data.get("expenses"):
        ...
        user_data["expenses"] = [
            {"name": "Rent", "target": 1200.0, "deadline": today.replace(day=30), "saved_so_far": 0.0},
            {"name": "Utilities", "target": 125.0, "deadline": today.replace(day=30), "saved_so_far": 0.0},
            {"name": "Electric", "target": 50.0, "deadline": today.replace(day=30), "saved_so_far": 0.0},
            {"name": "Internet", "target": 30.0, "deadline": today.replace(day=30), "saved_so_far": 0.0},
            {"name": "Auto Insurance", "target": 500.0, "deadline": today + relativedelta(months=+6), "saved_so_far": 0.0},
            {"name": "Renters Insurance", "target": 150.0, "deadline": today + relativedelta(months=+12), "saved_so_far": 0.0},
        ]
        save_users(users)
    ...
    # --- Paycheck & Frequency Input ---
    st.subheader("Your Paycheck Info")
    col1, col2 = st.columns([2,3])
    with col1:
        paycheck = st.number_input("Enter this paycheck amount", min_value=0.0, step=25.0, value=0.0)
    with col2:
        additional_income = st.number_input("Any extra income this period?", min_value=0.0, step=25.0, value=0.0)
    total_paycheck = paycheck + additional_income

    pay_frequency = st.selectbox("Pay frequency", ["Weekly", "Bi-weekly", "Monthly"], index=1)
    ...
    for i, goal in enumerate(st.session_state.expenses):
        col1, col2, col3, col4, col5 = st.columns([3,2,3,1,2])
        with col1:
        ...
        with col4:
            st.session_state.expenses[i]["saved_so_far"] = st.number_input(
                "Saved So Far", min_value=0.0, value=float(goal["saved_so_far"]), step=10.0, key=f"saved_{i}"
            )
        with col5:
    ...
    df["days_until_deadline"] = (df["deadline"] - pd.to_datetime(datetime.date.today())).dt.days
    df["paychecks_left"] = (df["days_until_deadline"] / days_between_paychecks).apply(lambda x: max(int(x)+1,1))
    # suggested allocation = remaining amount / paychecks left
    df["per_paycheck"] = (df["target"] - df["saved_so_far"]) / df["paychecks_left"]

    total_allocations = df["per_paycheck"].sum()
    leftover = max(total_paycheck - total_allocations, 0)

    # --- Display Allocations ---
    st.subheader("Allocations This Paycheck")
    st.dataframe(df[["name","target","deadline","saved_so_far","paychecks_left","per_paycheck"]])
